...
def gradient_check():
    ...
    nn = NeuralNetwork(input_size=2, hidden1=4, hidden2=3, output_size=1, activation='tanh')
    nn.forward(x)
...
# Transpose for neural network (features x samples)
X_train = X_train.T
X_test = X_test.T
...
for activation in activations:
    ...
    model = NeuralNetwork(input_size=2, hidden1=16, hidden2=8, output_size=1, activation=activation)
    model.train(X_train, y_train, epochs=1000, learning_rate=0.01, batch_size=32)

    predictions = model.predict(X_test)
    accuracy = np.mean(predictions == y_test) * 100  # Convert to percentage
    results[activation] = accuracy

    print(f"{activation.upper()} Accuracy = {accuracy:.2f}%")
...
best_model = NeuralNetwork(input_size=2, hidden1=16, hidden2=8, output_size=1, activation=best_activation)
best_model.train(X_train, y_train, epochs=1000, learning_rate=0.01, batch_size=32)

# Plot learning curve
plt.figure(figsize=(8, 5))
plt.plot(best_model.loss_history, linewidth=2)
...
for i in range(10):
    actual = "Class 1" if y_test[0, i] == 1 else "Class 0"
    predicted = "Class 1" if final_predictions[0, i] == 1 else "Class 0"
    status = "✓" if y_test[0, i] == final_predictions[0, i] else "✗"
    print(f"Sample {i+1:2d} | Actual: {actual:7s} | Predicted: {predicted:7s} | {status}")
