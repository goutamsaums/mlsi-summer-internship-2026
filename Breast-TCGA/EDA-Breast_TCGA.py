# -*- coding: utf-8 -*-
"""EDA_Breast-TCGA.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1VNh4pUBCv_fpb91p3yUk3ZGtHCBFEDaj
"""

# Commented out IPython magic to ensure Python compatibility.
# ============================================================
# RUN TCGA BRCA EDA PIPELINE
# ============================================================
...
!pip install -q statsmodels umap-learn networkx

# Step 4: Run the pipeline (paste the entire code above and execute)
# ============================================================
...
# 2. Feature-Level Analysis (Range, Variance, Correlation)
...
# 2.2 VARIANCE ANALYSIS (NEW - Publication Quality)
# ============================================================

print("\n" + "="*60)
print("2.2 VARIANCE ANALYSIS")
print("="*60)
...
for name, df in datasets.items():
    ...
    # Top high variance features
    top_var = variances.sort_values(ascending=False).head(10)
    print(f"\n📊 {name} - Top 10 High Variance Features:")
    print(top_var.to_string())

    # Variance distribution histogram
    plt.figure(figsize=(10, 5))
    plt.hist(variances, bins=50, edgecolor='black', alpha=0.7)
    plt.axvline(variances.mean(), color='red', linestyle='--', label=f'Mean: {variances.mean():.4f}')
    plt.axvline(variances.median(), color='green', linestyle='--', label=f'Median: {variances.median():.4f}')
    plt.xlabel("Variance")
    plt.ylabel("Number of Features")
    plt.title(f"{name} - Variance Distribution")
    plt.legend()
    plt.tight_layout()
    plt.show()
...
variance_df = pd.DataFrame(
    variance_summary,
    columns=["Modality", "MeanVariance", "MedianVariance", "MaxVariance", "MinVariance", "StdVariance"]
)
print("\n📊 Variance Analysis Summary:")
print(variance_df.to_string(index=False))

# 2.3 SKEWNESS & KURTOSIS ANALYSIS
# ============================================================

print("\n" + "="*60)
print("2.3 SKEWNESS & KURTOSIS ANALYSIS")
print("="*60)
...
# 2.4 CORRELATION ANALYSIS (With Clustermap)
# ============================================================

print("\n" + "="*60)
print("2.4 CORRELATION ANALYSIS (Within Modality)")
print("="*60)
...
for name, df in datasets.items():
    ...
    # NEW: Clustermap - automatically groups correlated genes
    plt.figure(figsize=(12, 10))
    sns.clustermap(corr, cmap="coolwarm", center=0, method='ward',
                   col_cluster=True, row_cluster=True,
                   figsize=(12, 10), cbar_pos=(0.02, 0.8, 0.03, 0.18))
    plt.title(f"{name} - Correlation Clustermap (Top 50 Features)")
    plt.tight_layout()
    plt.show()

    # Regular heatmap (for comparison)
    plt.figure(figsize=(12, 10))
...
# PAM50 vs Omics with Violin Plots (NEW)
if "PAM50 mRNA" in clinical_matched.columns:
    print("\n📊 PAM50 vs Omics (Boxplots + Violin Plots):")
    ...
    for name, df in datasets.items():
        ...
        for i, feature in enumerate(top_features):
            if i < len(axes):
                ...
                axes[i].set_title(f"Boxplot: {feature[:30]}")
                axes[i].tick_params(axis='x', rotation=45)

        plt.suptitle(f"{name} - PAM50 vs Top 5 Features (Boxplots)", fontsize=16)
        plt.tight_layout()
        plt.show()

        # Violin plots (NEW - shows density distribution)
        fig, axes = plt.subplots(1, 5, figsize=(20, 5))
        axes = axes.flatten()

        for i, feature in enumerate(top_features):
            if i < len(axes):
                sns.violinplot(x=y, y=df[feature], ax=axes[i])
                axes[i].set_title(f"Violin: {feature[:30]}")
                axes[i].tick_params(axis='x', rotation=45)

        plt.suptitle(f"{name} - PAM50 vs Top 5 Features (Violin Plots)", fontsize=16)
        plt.tight_layout()
        plt.show()
...
# Use top 500 features from mRNA and Methylation
...
# 4.2 t-SNE (NEW - Nonlinear Dimensionality Reduction)
# ============================================================

print("\n" + "="*60)
print("4.2 t-SNE ANALYSIS (Nonlinear)")
print("="*60)
...
plt.xlabel("t-SNE 1")
plt.ylabel("t-SNE 2")
plt.title("t-SNE Visualization - Colored by PAM50 Subtype")
plt.legend()
plt.tight_layout()
plt.show()
...
# 4.4 CROSS-MODALITY RELATIONSHIPS (PC1 based)
# ============================================================

print("\n" + "="*60)
print("4.4 CROSS-MODALITY RELATIONSHIPS (PC1 based)")
print("="*60)
